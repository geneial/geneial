			...
			)
{
			...
			++choosenChromosomeContainer_it)
	{
			...
			std::copy(mutantChromosomeContainer.begin(),mutantChromosomeContainer.end(),std::back_inserter(result_container));
			...
			else
			{
				...
				for(typename value_container::iterator it = mutatedChromosome->getContainer().begin();it != mutatedChromosome->getContainer().end();++it)
				{
			...
			Smoothing<VALUE_TYPE,FITNESS_TYPE>::restoreSmoothness(
					...
					this->getBuilderSettings()->getRandomMax()
					);
