...
inline typename BaseCrossoverOperation<FITNESS_TYPE>::crossover_result_set MultiValueChromosomeBlendingCrossover<VALUE_TYPE, FITNESS_TYPE>::doCrossover(
	...
	for(unsigned int i = 0; i< chromosomesToGenerate;i++)
	{
				...
				++i)
		{
			...
			switch(interpolationMethod)
			{
				...
				{
					targetBeta = Interpolators::cosineInterpolate<double>(beta1,beta2,((double)i)/((double) containerSize));
					break;
				}
				case INTERPOLATE_LINEARLY:
				{
					targetBeta = Interpolators::linearInterpolate<double>(beta1,beta2,((double)i)/((double) containerSize));
					break;
				}
			...
			assert(targetBeta >= 0);
			child_container.push_back(
					(*mommy_it * (targetBeta) + *daddy_it * (targetBeta)));
			++mommy_it;
			++daddy_it;
		}
