...
public:
	SmoothedMultiValueChromosomeNPointCrossover(
			...
			) : MultiValueChromosomeNPointCrossover<VALUE_TYPE,FITNESS_TYPE>(crossoverSettings,builderSettings,builderFactory),
			...
			typename BaseChromosome<FITNESS_TYPE>::ptr daddy)
	{
		...
		for(typename BaseCrossoverOperation<FITNESS_TYPE>::crossover_result_set::iterator it = result.begin();
			...
			Smoothing::restoreSmoothness<VALUE_TYPE,FITNESS_TYPE>(
					...
					_builderSettings->getEps(),
					_builderSettings->getRandomMin(),
					_builderSettings->getRandomMax()
					);
